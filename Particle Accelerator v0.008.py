...
radius_button = pygame.Rect(10, 10, 300, 50)
lifetime_button = pygame.Rect(10, 70, 300, 50)
speed_button = pygame.Rect(10, 130, 300, 50)
rainbow_button = pygame.Rect(10, 190, 328, 50)
time_multiplier_button = pygame.Rect(10, 250, 300, 50)
reset_button = pygame.Rect(10, 310, 300, 50)

# Button colors
radius_button_color = (128, 0, 128)  # Color for Radius button
...
# Text labels
radius_text = font.render(f"Particle Radius: {PARTICLE_RADIUS}", True, (255, 255, 255))  # Initial Radius text
...
brightness_slider_y = SCREEN_HEIGHT - 60
...
color_slider_x = 350
color_slider_y = SCREEN_HEIGHT - 30
...
def update_particles():
    ...
    for particle in particles:
        particle['x'] += particle['vx'] * time_multiplier  # Apply Time Multiplier to particle movement
        particle['y'] += particle['vy'] * time_multiplier  # Apply Time Multiplier to particle movement
        particle['lifetime'] -= 1
...
# Define a function to display the FPS
def display_fps():
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if radius_button.collidepoint(event.pos):
                # Handle radius button clicks
                if event.button == 4:  # Scroll up to increase radius
                    PARTICLE_RADIUS = min(PARTICLE_RADIUS + 1, 1000)  # Cap the radius at 1000
                elif event.button == 5:  # Scroll down to decrease radius
                    if PARTICLE_RADIUS >= 2:
                        PARTICLE_RADIUS -= 1
                radius_text = font.render(f"Particle Radius: {PARTICLE_RADIUS}", True, (255, 255, 255))
            elif lifetime_button.collidepoint(event.pos):
                # Handle lifetime button clicks
                if event.button == 4:  # Scroll up to increase lifetime
                    particle_lifetime = min(particle_lifetime + 10, 999)  # Limit to 999
                ...
                if event.button == 4:  # Scroll up to increase speed
                    particle_speed = min(particle_speed + 1, 1000)  # Limit to 1000
                ...
                red_value = 0  # Reset red value
                green_value = 0  # Reset green value
                blue_value = 0  # Reset blue value
                time_multiplier = 1  # Reset time multiplier
    ...
    change_button_color(radius_button, radius_button_color)  # Color for Radius button
    ...
    if radius_button.collidepoint(pygame.mouse.get_pos()):
        pygame.draw.rect(screen, (160, 32, 240), radius_button, border_radius=10)  # Highlight Radius button
    ...
    text_color = (255 - background_color[0], 255 - background_color[1], 255 - background_color[2])
    bg_color_text = font.render("Background color", True, text_color)
    ...
    screen.blit(radius_text, (15, 20))  # Display Radius text at the top
    screen.blit(lifetime_text, (15, 80))
    screen.blit(speed_text, (15, 140))
    screen.blit(reset_text, (15, 320))
